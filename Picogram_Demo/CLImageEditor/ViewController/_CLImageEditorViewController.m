...
- (void)initNavigationBar
{
    ...
    if(![doneBtnTitle isEqualToString:@"CLImageEditor_DoneBtnTitle"]){
        rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Next" style:UIBarButtonItemStyleDone target:self action:@selector(pushedFinishBtn:)];
        rightBarButtonItem.tintColor = [UIColor colorWithRed:0.1176 green:0.1176 blue:0.1176 alpha:1.0];

        leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"comments_back_icon_on"] style:UIBarButtonItemStyleDone target:self action:@selector(pushedBackBtn:)];
    }
    else{
        rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(pushedFinishBtn:)];
    }
    ...
    if(_navigationBar==nil){
        ...
        navigationItem.leftBarButtonItem.tintColor = [UIColor colorWithRed:0.2549 green:0.5137 blue:0.8431 alpha:1.0];
...
- (void)initMenuScrollView
{
    ...
    if(self.menuView==nil){
        UIScrollView *menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0,0, self.view.width, 200)];//80
...
- (void)resetImageViewFrame
{
    ...
    if(size.width>0 && size.height>0){
        ...
        _imageView.frame = CGRectMake(MAX(0, (_scrollView.width-W)/2),0, W, H+5);
    }
}
...
- (void)swapToolBarWithEditting:(BOOL)editting
{
    ...
    if(self.currentTool){
        ...
        item.leftBarButtonItem  = [[UIBarButtonItem alloc] initWithTitle:@"Back"/*[CLImageEditorTheme localizedString:@"CLImageEditor_BackBtnTitle" withDefault:@"Back"]*/ style:UIBarButtonItemStylePlain target:self action:@selector(pushedCancelBtn:)];
        
        //item.leftBarButtonItem.tintColor = [UIColor colorWithRed:0.2549 green:0.5137 blue:0.8431 alpha:1.0];
...
- (void)pushedFinishBtn:(id)sender
{
    if(self.targetImageView==nil){
        if([self.delegate respondsToSelector:@selector(imageEditor:didFinishEdittingWithImage:)]){
            ...
            UIStoryboard *oldstorybord = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
            PGShareViewController *ShareVC = [oldstorybord  instantiateViewControllerWithIdentifier:@"photoSelected"];
            ShareVC.image2 = _originalImage;
           ...
           // NSData *data = UIImagePNGRepresentation(_originalImage);
            ...
            NSData *imgData1 = UIImageJPEGRepresentation(_originalImage, 0.7f);
            ...
            UIImage  *selectedProfileThumbNailImage = [self imageWithImage:_originalImage scaledToSize:CGSizeMake(50,50)];
            //NSData *thumbNaildata = UIImagePNGRepresentation(selectedProfileThumbNailImage);
            NSData *thumbImgData = UIImageJPEGRepresentation(selectedProfileThumbNailImage, 0.7f);
            NSLog(@"0.7 size: %lu", (unsigned long)thumbImgData.length);
            ...
            ShareVC.postedImagePath = imagePath;
            ShareVC.postedthumbNailImagePath = ThumbnailimagePath;
            
            UINavigationController *navBar =[[UINavigationController alloc]initWithRootViewController:ShareVC];
            navBar.navigationBar.barTintColor= [UIColor blackColor];
