...
//#import "LayerAuthenticationViewController.h"
#import "DirectChatViewController.h"
#import "DirectChatTableViewCell.h"
#import "UsersVC.h"
...
@interface DirectChatViewController () <UITableViewDataSource,UITableViewDelegate,CouchBaseEventsDelegate,CBLDocumentModel,UISearchBarDelegate>
...
- (void)createNavRightButton {
    ...
    [navCancelButton setImage:[UIImage imageNamed:@"direct_add_icon_off"]
                     forState:UIControlStateNormal];
    [navCancelButton setImage:[UIImage imageNamed:@"direct_add_icon_on"]
...
-(void)messageTypeButtonClicked {
    ...
    UIActionSheet *acctionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Send Photo or Video",@"Send Message", nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    __weak DirectChatTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    ...
    NSString *imageURL = [nameImagArr lastObject];
    
    if ([getDocument.properties objectForKey:@"groupID"]) {
        imageURL = [NSString stringWithFormat:@"%@",[getDocument.properties objectForKey:@"groupPic"]];
    }
    ...
    if(imageURL==nil || imageURL==(id)[NSNull null] || [imageURL isEqualToString:@"(null)"])
    {
        ...
        cell.friendsImage.image = placeholderImage;
        cell.self.friendsImage.layer.cornerRadius = cell.self.friendsImage.frame.size.width / 2;
        cell.self.friendsImage.clipsToBounds = YES;
        [cell setNeedsLayout];
...
//    cell.batchCountLabelOutlet.text = [[NSUserDefaults standardUserDefaults] objectForKey:row.documentID];
    cell.userName.text =[self getReciverName:[nameImagArr firstObject]];
    cell.msgLbl.text = [self filterLastmsg:getDocument];
    cell.postImage.hidden = YES;
    
    if ([getDocument.properties objectForKey:@"groupID"]) {
        
        cell.userName.text =[self getReciverName :[NSString stringWithFormat:@"%@",[getDocument.properties objectForKey:@"groupName"]]];
    }
...
#pragma mark - Helpers
- (IBAction)newChatActionButton:(id)sender {
    
    [[NSUserDefaults standardUserDefaults]setBool:YES forKey:newChatBtnCliked];
    [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"CheckContacFirstTime"];
    [self performSegueWithIdentifier:@"newChat" sender:nil];
}
...
//        
//        FavoritesTableViewController *fav = [segue destinationViewController];
//        // fav.isComingFromChatList = YES;
//        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"isComingFromChat"];
//        
//    }
